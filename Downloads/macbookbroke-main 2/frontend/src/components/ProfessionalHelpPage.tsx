...
const ProfessionalHelpPage: React.FC = () => {
  ...
  const requestLocationPermission = async () => {
    ...
    try {
      ...
      setError(err.message || t('failed_to_get_location'));
  ...
  const fetchNearbyHelplines = async (location: UserLocation) => {
    try {
      ...
      setError(t('failed_to_fetch_helplines'));
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className={`${isDark ? 'bg-gray-800' : 'bg-blue-50'} rounded-lg p-4 mb-6`}>
            <div className="flex items-center">
              ...
              <span className={`${isDark ? 'text-gray-300' : 'text-blue-800'} text-sm`}>
                {t('location_label')}: {userLocation.latitude.toFixed(4)}, {userLocation.longitude.toFixed(4)}
              </span>
            </div>
          </div>
        )}
                  ...
                  <div className="flex-1">
                    ...
                    {center.distance && (
                      <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-sm`}>
                        {center.distance.toFixed(1)} {t('km_away')}
                      </p>
                    )}
                  </div>
                </div>
