...
export function computeWRI(inputs: NormalizedInputs, history?: Array<{ date: string; wri: number }>, applyJournalBonus: boolean = true): WriOutput {
  ...
  let totalMeditationBonus = 0;
  let totalGoalBonus = 0;
  ...
  if (applyJournalBonus) {
    ...
    // Get meditation bonus from inputs (passed from backend)
    const meditationWRIReduction = (inputs as any).meditation_wri_reduction || 0;
    if (meditationWRIReduction > 0) {
      totalMeditationBonus = meditationWRIReduction;
      console.log(`🧘‍♀️ Meditation WRI reduction: ${meditationWRIReduction} points`);
    }

    // Get goal completion bonus from inputs
    const goalWRIReduction = (inputs as any).goal_wri_reduction || 0;
    if (goalWRIReduction > 0) {
      totalGoalBonus = goalWRIReduction;
      console.log(`🎯 Goal completion WRI reduction: ${goalWRIReduction} points`);
    }
    ...
    // Apply meditation bonus (separate from journal bonus)
    if (totalMeditationBonus > 0) {
      console.log(`🧘‍♀️ TOTAL MEDITATION BONUS: ${totalMeditationBonus} points reduction (WRI: ${wri} → ${Math.max(0, wri - totalMeditationBonus)})`);
      wri = Math.max(0, wri - totalMeditationBonus);
    }

    // Apply goal completion bonus (separate from journal and meditation)
    if (totalGoalBonus > 0) {
      console.log(`🎯 TOTAL GOAL COMPLETION BONUS: ${totalGoalBonus} points reduction (WRI: ${wri} → ${Math.max(0, wri - totalGoalBonus)})`);
      wri = Math.max(0, wri - totalGoalBonus);
    }
  } else {
...
export async function calculateJournalMetrics(userEmail: string): Promise<{
  ...
  try {
    ...
    const response = await fetch(`${apiConfig.journal}?action=list`, {
      method: 'POST',
      headers: { 
...
export async function storeJournalInsights(userEmail: string, journalMetrics: {
  ...
  try {
    ...
    const response = await fetch(`https://us-central1-YOUR_PROJECT_ID.cloudfunctions.net/storeJournalInsights`, {
      method: 'POST',
...
export async function getEncryptedJournalInsights(userEmail: string): Promise<{
  ...
  try {
    ...
    const response = await fetch(`https://us-central1-YOUR_PROJECT_ID.cloudfunctions.net/getJournalInsights`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
